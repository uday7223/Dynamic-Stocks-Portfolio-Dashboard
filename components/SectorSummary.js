const SectorSummary = ({ stocks }) => {
    const sectors = stocks.reduce((acc, stock) => {
      const { sector, investment, presentValue } = stock;
      ...
      acc[sector].totalValue += validPresentValue; // Assuming presentValue is the current price per share
    ...
    return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {Object.entries(sectors).map(([sector, summary]) => (
            <div key={sector} className="p-4 bg-gray-800  shadow-md">
